...
/** This class implements the texture synthesis algorithm described
 *  by the paper "Image Quilting for Texture Synthesis and Transfer".
 *  This only knows how to make a new texture image (ie it
 *  does not implement hole filling or other uses of the algorithm).
 *
 *  This class adds some optional extensions to improve the quality
 *  of the output given more run time.
 */
public class ImageQuilter {

    private RGBIntegerImage input;
     ...
     *  @param allowHorizontalPaths When finding min paths,
     *                              can the path travel along a stage?
     *  @param pathCostWeight The SSD for the overlap region and the
     *                        min SSD path cost have the same range. The total
     *                        cost is then pathCost*pathCostWeight
     *                        plus ssd*(1-pathCostWeight).
     */
    public ImageQuilter(RGBIntegerImage input, int patchsize, int overlapsize,
     ...
     */
    public RGBIntegerImage synthesize(int outwidth, int outheight) {
	...
	RGBIntegerImage output = null;
	output = (RGBIntegerImage) input.createCompatibleImage(outwidth,
							       outheight);
	output.clear(0);
	...
	if( outPatch.isAtLeftEdge() ) {
		...
		for(int c = 0; c < overlapsize; c++) {
		    if( where[r][c] ) {
			outPatch.putSample( c, r, inPatch.getSample(c,r) );
		    }
		}
	    }
