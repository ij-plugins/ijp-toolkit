...
public class MinPathFinder {
    ...
    /** costs[i][j] gives the cumulative cost of going from spot i,j to the
     *  destination.
     */
    private double costs[][];

    /** destinations[i][j] gives the spot to go next from spot i,j when
     *  heading toward the destination.
     */
    private TwoDLoc path[][];
    
    /** This sets up a path finder to find the min path from last row
     *  to the first row given the cost of being at each position.
     *
     *  @param dists This gives the cost of each position where dists[0]
     *               is the row of destination locations and
     *               dists[dists.length-1] is the row of starting positions.
     *               This must be a rectangular array of non-zero dimensions.
     *  @param allowHorizontal This says whether or not to allow the
     *                         path to follow along a row if it wants to.
     */
    ...
    public TwoDLoc[][] getPaths() { return path; }
