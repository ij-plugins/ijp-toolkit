 ...
 * [[ProgressReporter4J ProgressReporter4J]] wrapper
...
trait ProgressReporter {
  ...
  /**
   * Notify listeners of the current value of progress.
  ...
  /**
   * Set a new progress value and notify listeners.
   *
   * @param progress new value of `currentProgress`. The value must be not less than 0 and not more than 1.
   * @param message  message that will be sent to listeners within `ProgressEvent`
   */
  ...
  /**
   * Set a new progress value and notify listeners. Progress is calculated from `count` and `max` values:
   * {{{
