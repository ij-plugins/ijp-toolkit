...
//            if (key < y.key) {
    ...
    private boolean remove(final Node<K> node) {
        if (node == Node.NULL) {
            return false;
        }

        final Node<K> y = node.left == Node.NULL || node.right == Node.NULL
                ? node
                : successor(node);

        final Node<K> x = y.left != Node.NULL
                ? y.left
                : y.right;
    ...
    private Node<K> select(final Node<K> x, final int i) {
    ...
    private void insertFixup(final Node<K> node) {
        Node<K> z = node;
        while (z.parent.color == RED) {
            if (z.parent == z.parent.parent.left) {
                final Node<K> y = z.parent.parent.right;
                if (y.color == RED) {
                ...
                final Node<K> y = z.parent.parent.left;
                if (y.color == RED) {
                    z.parent.color = BLACK;
                    y.color = BLACK;
                    z.parent.parent.color = RED;
    ...
    private void rightRotate(final Node<K> x) {
        final Node<K> y = x.left;
        x.left = y.right;
    ...
    private void leftRotate(final Node<K> x) {
        final Node<K> y = x.right;
        x.right = y.left;
     ...
     */
    private Node<K> successor(final Node<K> node) {
        Node<K> x = node;
    ...
    private Node<K> treeMinimum(final Node<K> node) {
        Node<K> x = node;
    ...
    private void deleteFixup(final Node<K> node) {
        Node<K> x = node;
    ...
    private Node<K> find(final Node<K> node, final K key) {
        Node<K> x = node;
    ...
    private void printTree(final Node<K> node) {
        if (node != Node.NULL) {
            printTree(node.left);
